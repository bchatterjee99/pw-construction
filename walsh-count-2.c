...
void num_largest()
{
	...
	if(112 - i <= 63)
	{
	    ...
	    {
		long new_target = reduced_targets[j] - take * coins[NUM_COINS - 1];
		if(new_target < 0)
		    printf("target %ld, possible: 0", new_target);
		else
		    printf("target %ld, possible: %d", new_target, old_table[NUM_COINS - 2][new_target]);
		printf("\n");
	    }
	    printf("\n\n");
	}
